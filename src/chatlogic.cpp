                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            std::unique_ptr<GraphEdge> edges = std::make_unique<GraphEdge>(id);
                            edges->SetChildNode((*childNode).get());
                            edges->SetParentNode((*parentNode).get());

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edges);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edges.get());
                            (*parentNode)->AddEdgeToChildNode(std::move(edges));
                        }
